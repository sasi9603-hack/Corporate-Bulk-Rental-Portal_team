...
export default function AdminSupport() {
  ...
  if (loading) return <LoadingSpinner text="Loading helpdesk..." />

  return (
    <div className="h-[calc(100vh-6rem)] flex flex-col animate-in">
      <div className="page-header mb-4">
        <div>
          <h1 className="section-title">Helpdesk & Support</h1>
          <p className="text-slate-500 text-sm mt-1">Manage client issues and maintenance requests</p>
        </div>
      </div>
            ...
            {tickets.length === 0 ? (
              ...
              tickets.map(t => (
                ...
                >
                  ...
                  <p className="text-xs text-slate-500 mt-1">{t.profiles?.full_name || t.profiles?.email}</p>
            ...
            <>
              ...
              <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50/50">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">Client: {activeTicket.profiles?.full_name}</p>
                </div>
                <div className="flex gap-2">
                  {activeTicket.status !== 'Closed' && (
                    <button onClick={() => updateTicketStatus(activeTicket.id, 'Closed')} className="btn-secondary text-xs py-1.5 px-3">
                      Mark as Closed
                    </button>
                  )}
                  {activeTicket.status === 'Closed' && (
                    <button onClick={() => updateTicketStatus(activeTicket.id, 'In Progress')} className="btn-secondary text-xs py-1.5 px-3">
                      Reopen Ticket
                    </button>
                  )}
                </div>
                  ...
                  return (
                    <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                      <span className="text-[10px] text-slate-400 mb-1 px-1">
                        {isMe ? 'You' : msg.profiles?.full_name} • {format(new Date(msg.created_at), 'h:mm a')}
                      </span>
